...
knitr::opts_chunk$set(
  echo = TRUE, message = FALSE, error = FALSE, size = "small")
...
library(lbfgs)
out.lbfgs <- lbfgs(objective, gradient, c(-1.2, 1), invisible = 1)
...
all.equal(out.lbfgs[c("value", "par")], out.optim[c("value", "par")], 
          tolerance = 1e-5)
@

The results are essentially the same, but \pkg{lbfgs} achieves better running speeds in a microbenchmark\footnote{%
All remarks about computation times are as of this writting. The results may differ with different compilers, hardware etc.} test done using the \pkg{microbenchmark} package \citeshort{microbenchmark}:

<<bench_first>>=
library(microbenchmark)
microbenchmark(
  lbfgs = lbfgs(objective, gradient, c(-1.2, 1), invisible=1), 
  optim = optim(c(-1.2, 1), objective, gradient, method="L-BFGS-B"))
@
...
<<logistic_asg_funcs>>=
likelihood <- l_logistic <- function(par, X, y, prec) {
  Xbeta <- X %*% par
  -(sum(y * Xbeta - log(1 + exp(Xbeta))) - .5 * sum(par^2*prec))
}

gradient <- g_logistic <- function(par, X, y, prec) {
  p <-  1/(1 + exp(-X %*% par))
...
<<logistic_estimate>>=
lbfgs.out <-lbfgs(likelihood, gradient, init, invisible=1, X=X1, y=y, 
                  prec=2)
...
In this particular case, \code{optim} outperforms \pkg{lbfgs}, but it is to be noted that this problem has a high number of parameters and a low number of observations:

<<logistic_bench>>=
microbenchmark(
  lbfgs = lbfgs(likelihood, gradient, init, invisible=1, X=X1, y=y, prec=2), 
  optim = optim(init, likelihood, gradient, method = "L-BFGS-B", X=X1, y=y, 
                prec=2), times = 10)
@
...
all.equal(out$par, as.vector(coef(fit, s = lambda_use)))
@
...
<<poisson_bench>>=
microbenchmark(
  lbfgs = lbfgs(likelihood, gradient, init, X=X1, y=y, prec=0, 
                invisible=1, orthantwise_c=C, 
                linesearch_algorithm="LBFGS_LINESEARCH_BACKTRACKING",
                orthantwise_start = 1,
                orthantwise_end = ncol(X1)),
  glmnet = glmnet(x, y, family="poisson", standardize=FALSE))
@
...
<<ass_R_again>>=
objective.R <- function(par, ...) {   
  100 * (par[2] - par[1]^2)^2 + (1 - par[1])^2
}

gradient.R <- function(par, ...) {
  c(-400 * par[1] * (par[2] - par[1]^2) - 2 * (1 - par[1]),
    200 * (par[2] - par[1]^2))
}
@
...
microbenchmark(
  ...
  optim =  
    out.optim <- optim(c(-1.2, 1), objective.R, gradient.R, 
                       method="L-BFGS-B"))

all.equal(out.CPP$par, out.R$par)
all.equal(out.CPP$par, out.optim$par, tolerance = 1e-5)
@
...
likelihood.CPP <- cxxfunction(
  signature(), body=likelihood.body, inc=likelihood.include, 
  plugin="RcppArmadillo")

gradient.CPP <- cxxfunction(
  signature(), body=gradient.body, inc=gradient.include, 
  plugin="RcppArmadillo")
@
...
microbenchmark(
  `lbfgs (C++)` = 
    out.CPP <- lbfgs(likelihood.CPP(), gradient.CPP(), invisible=1, init, 
                     environment=env), 
  `lbfgs (R)` = 
    out.R <- lbfgs(l_logistic, g_logistic, init, invisible=1, X=X1, y=y, 
                   prec=1), times = 10)

all.equal(out.CPP, out.R)
@
